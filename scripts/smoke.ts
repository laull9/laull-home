...
try {
  ...
  const anonEnginesRes = await request('/api/v1/search/engines')
  assert(anonEnginesRes.status === 401, '未登录访客访问搜索引擎未被拦截')
  const anonGroupsRes = await request('/api/v1/bookmarks/groups?spaceId=default')
  assert(anonGroupsRes.status === 401, '未登录访客访问书签分组未被拦截')
  const enginesRes = await request('/api/v1/search/engines', 'GET', undefined, cookie)
  assert(enginesRes.status === 200, '登录后读取搜索引擎失败')
  const authGroupsRes = await request('/api/v1/bookmarks/groups?spaceId=default', 'GET', undefined, cookie)
  assert(authGroupsRes.status === 200, '登录后读取书签分组失败')
