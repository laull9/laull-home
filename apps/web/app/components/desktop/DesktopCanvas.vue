...
import { arrangeNodes, newWidget, BREAKPOINTS, isWidget, scopedCss, type WidgetNode, type Bookmark, type BookmarkGroup, type Breakpoint, type Placement } from '@laull-home/shared'
...
const contextItems = computed(() => [
  ...
  { id: 'add', label: '添加图标导航' },
  { id: 'library', label: showComponentTree.value ? '收起组件树' : '添加组件' },
...
function contextAction(id: string) {
  ...
  emit('startEdit')
  if (id === 'library') showComponentTree.value = !showComponentTree.value
}
// 保存新书签后在当前桌面创建 1x1 图标组件，保持非编辑状态。
function addNavigation(bookmark: Bookmark) {
  ...
  add('bookmark', {
    ...
    layouts: { desktop: { x: 0, y: 0, w: 1, h: 1, pinned: false } },
  })
  if (!props.editing) void save()
}
// 供外部控制组件树显隐与右键菜单。
...
function handleAddWidget(
  ...
  add(type, undefined, variant, size, { frameless, referenceId: targetRefId, title: targetTitle })
}
...
async function drop(event: DragEvent) {
  ...
  if (!props.editing || saving.value || !data.value || !text.startsWith('new:')) return
  event.preventDefault()
...
function handleWidgetUpdate(node: WidgetNode) {
  update(node)
  if (!props.editing) {
...
// 响应配置弹窗保存并在非编辑模式下自动保存。
function handleEditorSave(node: WidgetNode) { update(node); if (!props.editing) void save() }
// 响应配置弹窗复制并在非编辑模式下自动保存。
function handleEditorCopy(node: WidgetNode) { add(node.type, node); if (!props.editing) void save() }
// 响应配置弹窗模板保存并在非编辑模式下自动保存。
function handleEditorTemplate(node: WidgetNode) { template(node); if (!props.editing) void save() }
// 响应配置弹窗删除并在非编辑模式下自动保存。
function handleEditorRemove(id: string) { remove(id); if (!props.editing) void save() }

onUnmounted(() => {
  if (autoSaveTimer) {
    clearTimeout(autoSaveTimer)
    if (!props.editing && dirty.value) void save()
  }
  observer?.disconnect()
...
<template>
  <div class="desktop" @contextmenu="openContext($event)">
    ...
    <!-- 主桌面画布网格 -->
    <div ref="canvas" class="desktop-grid" :class="{ editing, saving: editing && saving, 'is-dragging': drag.draggingId.value }" :style="{ '--columns': BREAKPOINTS[breakpoint] }" @dragover="libraryOver" @drop="drop($event)" @click.capture="drag.click">
...
.widget {
  ...
  border: var(--widget-border, 1px) solid var(--lh-border);
  border-radius: var(--widget-radius, var(--lh-radius-lg));
  background: color-mix(in srgb, var(--widget-surface, var(--lh-surface-solid, white)) var(--widget-opacity, var(--lh-surface-opacity, 92%)), transparent);
  color: var(--widget-text, var(--lh-text));
  backdrop-filter: blur(var(--widget-blur, var(--lh-blur))) saturate(160%);
  -webkit-backdrop-filter: blur(var(--widget-blur, var(--lh-blur))) saturate(160%);
  box-shadow: inset 0 1px 1px 0 var(--lh-glass-border, transparent), var(--lh-shadow-card);
  transition: transform 0.22s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.22s cubic-bezier(0.16, 1, 0.3, 1), background-color 0.2s, color 0.2s, border-radius 0.2s;
}
:root[data-theme="modern"] :not(.editing) .widget:not(.frameless-widget):not(.search-widget):hover {
...
.widget-tools { position: absolute; top: -12px; right: 4px; z-index: 6; display: flex; border: 1px solid var(--lh-border); background: color-mix(in srgb, var(--lh-bg) 88%, transparent); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-radius: 6px; box-shadow: var(--lh-shadow-sm); }
