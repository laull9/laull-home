...
export function useDesktop() {
  ...
  function update(node: WidgetNode) {
    if (!data.value || saving.value) return
    data.value.nodes = data.value.nodes.map(item => item.id === node.id ? node : item)
    dirty.value = true
  }
  // 添加目录组件或模板副本。
  function add(type: WidgetNode['type'], template?: WidgetNode) {
    if (!data.value || saving.value) return
    if (data.value.nodes.length >= 120) { error.value = '最多保存 120 个组件'; return }
    const node: WidgetNode = template ? JSON.parse(JSON.stringify(template)) : newWidget(type, crypto.randomUUID())
