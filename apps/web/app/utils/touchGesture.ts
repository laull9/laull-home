...
// 文件夹内图标长按拾起前的最大允许位移（像素），超出即视为滚动而不拾起。
export const TOUCH_PICK_TOLERANCE = 8
...
// 触屏按压判定结果：wait 继续等待、drag 进入拖动、expired 已交给长按菜单。
export type TouchPressOutcome = 'wait' | 'drag' | 'expired'
...
// 判定按压等待期间的移动：点按确定时间走完之前一律保持等待（描边线未闭合不许拖动），
...
export function resolveTouchPress(
  ...
  if (elapsedMs < confirmMs) return 'wait'
  return distance >= threshold ? 'drag' : 'wait'
}

// 判定文件夹内图标的拾起：确定时间走完之前不拾起，可滚动来源的位移超过容差即让位给滚动；
...
export function resolveTouchPick(
  ...
  if (!held) return mode === 'scrollable' && distance > tolerance ? 'scroll' : 'wait'
